 ...
 * 计算字数
 */
function calculateWordCount(stats) {
 ...
 * 统计单个文件的字数
 */
export async function countWordsInFile(filePath) {
    try {
 ...
 * 统计多个文件的字数
 */
export async function countWordsInFiles(filePaths) {
    const results = await Promise.all(filePaths.map(filePath => countWordsInFile(filePath)));
    return results;
}
